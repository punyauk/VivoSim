...
//
// NEW TEXT (to be added to language notecards...)
...
float VERSION = 5.9;    // 13 September 2022
...
string CHATTY = "";                        // CHATTY=Chatty      Animal can be 'Chatty, 'No chat', 'Silent' which is represented by  1, 0, -1 If set to 0, during day animal will play sounds but won't speak (I'm thirsty etc) & during night it won't make any sounds at all.
string RADIUS = "";                        // RADIUS=10          How far (in m) the animal will wander from its home position 
string LABELTYPE = "";                     // TEXT=Long          LabelType = 0; // 0= Long  1== Short  2== Off 
string SLEEP_MODE = "";                    // SLEEP=0            Set to 1 to have animal sleep at night (i.e. stay in down position and no sounds or chat)
string AN_AUTO_POO = "";                   // AN_AUTO_POO=0      Set to 1 to have all animals that give manure auto rez it when ready
...
string restrictAccess = "";                // RESTRICT_ACCESS=0  Set to 1 to only allow owner to select 'Butcher', 'Mate' and 'Follow' options
...
list animalButtons()
{
    ...
    if (mode == "upgradingAnimals") buttons = [TXT_ALL] + buttons;
    return buttons;
}
...
setIdleText()
{
    ...
    if (VER == -1) tmp = "[ V_ALL"; else tmp = "[ V_" +qsFloat2String((float)VER*0.1, 1, FALSE);
...
default
{
    ...
    object_rez(key id)
    {
        ...
        messageObj(id, "INIT|" +PASSWORD +"|0|0|ANIMAL|" +languageCode+"|" +SURFACE+"|" +(string)CHATTY +"|"+(string)LABELTYPE +"|"+(string)RADIUS +"|" +(string)AN_AUTO_POO +"|" +FEEDER +"|" +(string)SLEEP_MODE +"|" +(string)USE_NC +"|" +(string)restrictAccess);
        llSetTimerEvent(0.5);
    }
