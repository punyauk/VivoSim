...
// *********************************************************
string defaultGroup="no start-up group specified!";      // name of a group (not the full card name!) to load by default (regardless of its permission setting)
                                    //
integer resetOnQuit=TRUE;           // TRUE = when no more sitters, reset the script (will also load default group again)
...
integer onlySeatedControls = FALSE;
...
doQuit()
{
    ...
    while (--i>=0)
    {
        ...
        if (who!=NULL_KEY)
        {
            if (0 && osIsNpc(who)) osNpcStand(who);
            else
            {
...
autoLoadGroup(integer neededPositions, integer doPlay)
{
    string name;
    integer k;
    for (k=1; k < llGetListLength(invGroups); k+=invGroupStride)
    {
        if (llList2Integer(invGroups, k)==neededPositions)
        {
            name=llList2String(invGroups, k+2);
            jump autoLoadEnd;
        }
    }
    @autoLoadEnd;
    if (name == "")
    {
       // llOwnerSay("No group found for "+(string)neededPositions+" positions");
        return;
    }
...
default
{
    state_entry()
    {
        ...
        llWhisper(0,"Initialization complete and ready to use");
    }
    on_rez(integer num)
    {
        llResetScript();
    ...
    changed (integer change)
    {
        ...
        integer neededPositions=i-l; //llGetListLength(seated);
            ...
            if (neededPositions>0 && neededPositions!= currentlySeated)
            {
                autoLoadGroup(neededPositions, 0);
                currentlySeated = neededPositions;
            }
                        ...
                        else if (indexToSit==-1)
                        {
                            ...
                            llOwnerSay("Sorry, there are no available positions for you to occupy. Please wait for someone to stand");
                            llUnSit(who);
                        }
                        else
                        {
                  ...
                  //      if ( !allowSoloNPC)
                   ...
                   //         osNpcStand(who);
                    //        positions=[]+llListReplaceList(positions,[NULL_KEY],i,i);
                     //   }
                        //osNpcStand(who);
                        //osNpcStopAnimation(who,llList2String(currentAn,3+i*3));
                        //osNpcStopAnimation(who,baseAn);
                    //}
                    //else
                    if (llListFindList(seated,[who])==-1)
                    {
                        /// Someone Stood!
                        if (myState=="EDIT")
                        {
    ...
    touch_start(integer num)
    {
        ...
        else if (onlySeatedControls && myState=="READY") llRegionSayTo(who,0,"Please sit to begin using me");
        ...
        {
            if (onlySeatedControls &  llListFindList(positions,[who])==-1) llRegionSayTo(who,0,"Only current users may access the controls. Please sit, then try again");
        ...
        else if (onlySeatedControls)
             llOwnerSay("ERROR! Unexpected state when touched. State is: "+myState);
    }
