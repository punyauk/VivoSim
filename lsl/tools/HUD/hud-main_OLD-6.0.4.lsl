/*
 * CHANGE LOG
 * Removed most llSleeps
 * Fixed but that when entering timeRate I forgot to convert from mins down to seconds!
 * Changed the elapsed time system for eating, drinking & bladder
 * Added %age numbers as floating text on HUD indicators, plus config notecard option to turn on or off
...
float  VERSION = 6.04;         // 12 MAY 2023
string subVersion = "BETA";        // Set to "" for release
string NAME = "VivoSim-HUD";
//
integer DEBUGMODE = TRUE;     // Set this if you want to force debug mode
...
setPaused()
{
   ...
   if (checkIndicator(statusIndicator) == TRUE)  messageObj(indicatorKey, "VISIBILITY|" +PASSWORD+ "|0" +"|" +indicatorColour);
...
startUp()
{
    ...
    if (llGetSubString(llGetObjectDesc(), 0, 0) != "H") systemReset();
    ...
    listen(integer c, string nm, key id, string m)
    {
        ...
        else
        {
            ...
            else if (m == TXT_RESUME)
            {
                ...
                if (checkIndicator(statusIndicator) == TRUE)  messageObj(indicatorKey, "VISIBILITY|" +PASSWORD+ "|1|" +indicatorColour);
                refresh();
            }
