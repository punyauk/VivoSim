...
makeRain(integer radius)
{
    if ((rainState == FALSE) && (rainChance >0))
    {
		...
		// returns float within values 5.0 to <10.0
		float randomFloat = 10.0 + llFrand(-5.0);

		// rainChance is 1 for low through 3 for high
		if ((rainChance == 1) && (randomFloat >8.0))
		{
			doRain = TRUE;
		}
		else if ((rainChance == 2) && (randomFloat >6.0))
		{
			doRain = TRUE;
		}
		else
