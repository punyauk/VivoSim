...
// ---------------------------------------
float version = 6.0;   //  10 February 2023
...
// INFO
// Allows people to sell certain items for Quintopia coins and also to check their balance. It will credit them points for time spent with HUD active.
...
string TXT_UNLINKED_OK="has been un-linked from your Quintopia account";
string TXT_MESSAGE1="If you had an account at quintopia.org, you would have just received some Quintos!";
string TXT_LINKED_OK="is now linked to your Quintopia account";
...
default
{
	...
	link_message(integer sender_num, integer num, string msg, key id)
	{
		...
		else if (cmd == "CMD_PLUSPNT")
		{
			...
			if (quinActive == TRUE)
			{
				postMessage("task=creditxp&data1=" + (string)userToPay + "&data2=Farm time" + "&data3=" +(string)num);
				dlgUser = NULL_KEY;
...
/*  THIS IS NOW ONLY DONE ON EXCHANGE
			else if (cmd == "MENU")
			{
				items = [];
				integer i;
...
*/
			...
			else if (cmd == "XPTOTAL")  
			{
				...
				// JSON is  {"current":{"id":2,"title":"Farm hand","points":50,"image":"https://quintopia.org/components/com_community/assets/badge_2.png","published":1,"progress":"0.00"}}
