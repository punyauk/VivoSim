...
float VERSION = 6.03;        //  2 April 2023
...
showText(string msg, vector colour)
{
    if (visible == TRUE)
    {
        if (textColour != ZERO_VECTOR) colour = textColour;

        if (collectiveTag != "") msg = collectiveTag +"\n - " +msg;
        ...
        else
        {
            ...
            if (llList2String(tk, 1) == PASSWORD)
            {
                ...
                else if (cmd == "VISIBILITY")
                {
                    ...
                    if (tmp != "")
                    {
                        if (llGetSubString(llToUpper(tmp), 0, 0) == "A") textColour = ZERO_VECTOR; else textColour = llList2Vector(tk, 3);
