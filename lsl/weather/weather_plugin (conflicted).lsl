	...
	if (llGetInventoryType("config") == INVENTORY_NOTECARD)
	{
		...
		for (i=0; i < llGetListLength(lines); i++)
		{
			...
			if (llGetSubString(line, 0, 0) != "#")
			{
				...
				{
					if (IsVector(val) == FALSE)
					{
						rainPrimSize = <20.0, 20.0, 35.0>;
					}
					else
					{
						rainPrimSize = (vector)val;
					}
				}
...
makeDarkClouds(integer lnkNum)
{
	...
	if (isMaster == TRUE)
	{
		llRegionSay(FARM_CHANNEL, "WR_CLOUDS|" +PASSWORD +"|" +(string)owner +"|-1");
	}
...
makeRain(integer radius)
{
	if ((rainState == FALSE) && (rainChance >0))
	{
		...
		if (doRain == TRUE)
		{
			...
			if (primLightning == TRUE)
			{
				llMessageLinked(LINK_SET, 1, "LIGHTNING", "");
			}
			else
			{
				llMessageLinked(LINK_SET, -1, "LIGHTNING", "");
...
makeFog()
{
	...
	if (isMaster == TRUE)
	{
		llRegionSay(FARM_CHANNEL, "WR_FOG|" +PASSWORD +"|" +(string)owner +"|1|");
	}
}
...
setEnvironment(string dayCycle)
{
	if ((controlEnvironment == TRUE) && (isMaster == TRUE))
	{
		...
		if ((dayCycle == ENV_FINE) && (isMaster == TRUE))
		{
		...
		if (channel == FARM_CHANNEL)
		{
			...
			if (llList2String(tk, 1) == PASSWORD)
			{
				...
				else if (cmd == "WC_PING")
				{
					if (isMaster == TRUE)
					{
						llRegionSay(FARM_CHANNEL, "WC_PONG|" +PASSWORD +"|" +(string)owner);
					}
				}
