...
float   VERSION = 6.034;    // 13 April 2024
	...
	if (llGetInventoryType("config") == INVENTORY_NOTECARD)
	{
		...
		for (i=0; i < llGetListLength(lines); i++)
		{
			...
			if (llList2String(tok,1) != "")
			{
				...
				else if (cmd == "USE_TEXTURE") useTexture = (integer)val;
...
default
{
	...
	timer()
	{
		...
		else if (status == "verifying-soldpnt")
		{
			...
			llRegionSayTo(userToPay, 0, TXT_COMMS_ERROR);
		...
		if (lookingFor == "all")
		{
			...
			if (sellItems == [])
			{
				if (sellItems == [])
				{
					if (lookingFor != "")
					{
						llRegionSayTo(userToPay, 0, TXT_NOT_FOUND);
					}
				}
	...
	no_sensor()
	{
		...
		{
		  if (lookingFor != "") llRegionSayTo(userToPay, 0, TXT_NOT_FOUND);
		}
		else
		{
