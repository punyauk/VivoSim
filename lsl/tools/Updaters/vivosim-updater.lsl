//  vivosim-updater.lsl
/**
This script is used to upgrade VivoSim items.
...
string FormatDecimal(float number, integer precision)
{
	...
	if (number < 0) rounded = number - roundingValue; else rounded = number + roundingValue;
...
state update
{
    state_entry()
    {
		...
		llSetTextureAnim(ANIM_ON | LOOP, 0, 4, 8, 0, 24, 25);
