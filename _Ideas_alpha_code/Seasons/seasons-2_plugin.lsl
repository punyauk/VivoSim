// SEASONS MONITOR - Multi prim plant
//  
...
integer lnkNumber = LINK_ALL_CHILDREN;
...
setConfig(string str)
{
    ...
    if (llList2String(tok,0) != "")
    {
            ...
            if (cmd == "LINK") lnkNumber = (integer)val;
    }
}
    ...
    listen(integer channel, string name, key id, string message)
    {
        if (channel == FARM_CHANNEL)
        {
            if ( message == "Spring" || message == "Summer" || message == "Autumn" || message == "Winter")
            {
                ...
                if (doChange == TRUE)
                {
                    llSetLinkAlpha(lnkNumber, sAlpha, ALL_SIDES);
                    llSetLinkColor(lnkNumber, sColour, ALL_SIDES);
                }
            }
        }
    }
