...
vector          gSitTarget_Pos = <2.7,-0.2,2.3>;
...
//integer     gGearCount;
string      gPhysEngDesc;
integer     gPhysEng=0; // if os function is off set this to 0=ODE| 1=Bullet
...
{
    string msg;
    // osGetPhysicsEngineType() will give either "BulletSim", "OpenDynamicsEngine" or "ubODE"
    // 0=ODE| 1=Bullet
    if  (osGetPhysicsEngineType() == "BulletSim")
    {
        gPhysEng = 1;
...
init_followCam(float degrees)
{
    llSetCameraParams([
                       ...
                       CAMERA_PITCH, 17.0,                // (-45 to 80) DEGREES
                       ...
                       CAMERA_FOCUS_OFFSET, <-13.10,0,0>   // <-10,-10,-10> to <10,10,10> METERS
                      ]);
