...
// CHANGE LOG
//  Fix for animals born as pets never become adult
//  Added NOCTURNAL option in config notecard
//  Added GIVE_PRODUCE option in config notecard
//  Updated version to use 6.nn
//  General code tidy
...
integer  CHATTY              = TRUE;                // Animal can be 'Chatty, 'No chat', 'Silent' which is represented by  1, 0, -1
integer  SLEEP_MODE          = 0;                   // Set to 1 to have animal sleep at night (i.e. stay in down position and no sounds or chat)
integer  IMMOBILE            = FALSE;               // Assume animal will move about
integer  RADIUS              = 10;                  // Assume 10m walk radius
integer  nocturnal           = FALSE;               // NOCTURNAL=0   Set to 1 if animal is nocturnal
string   AN_FEEDER           = "SF Feeder";         // FEEDER=
string   AN_BAAH             = "Ah";                // CRY=Ah
integer  AN_HASGENES         = FALSE;               // 1 if animal passes genes through mating
integer  giveProducts        = TRUE;                // GIVE_PRODUCE=1		Set to 0 to not give any products
integer  AN_HASMILK          = FALSE;               // True if gives milk
...
integer  AN_AUTO_POO         = 1;                   // if 1, animal will rez poo when ready, if 0 then farmer needs to collect it.
...
integer  LIFETIME            = 2592000;             // LIFEDAYS=30              2592000 = 30 days
...
integer  restrictAccess      = FALSE;               // RESTRICT_ACCESS=0   Set to 1 to only allow owner to select 'Butcher', 'Mate' and 'Follow' options
...
string   TXT_BECOME_ADULT="I become adult at around";
string   TXT_BUTCHER="Butcher";
string   TXT_CHANGE_NAME="New Name";
...
string   TXT_EAT_FROM="I eat from";
string   TXT_EGG="Egg";
...
string   TXT_INFO_MSG="Visit https://vivosim.net/help/animals  for more information.";
...
string   TXT_RESTRICTED="Restricted";
string   TXT_REZ="Rez";
...
string   SUFFIX;
list     colorable = [];
list     feeders = [];     // List of all feeders, read from an_feeders notecard
list     petResponses;
float    happy = 100.;
string   PASSWORD = "*";
...
setConfig(string str)
{
	...
	if (llList2String(tok,0) != "")
	{
		...
		else if (cmd == "GIVE_PRODUCE") giveProducts = (integer)val;
		...
		else if (cmd == "ADULT_RANDOM_PRIMS") ADULT_RANDOM_PRIMS = llParseString2List(val, [","] , []);
		...
		else if (cmd == "RESTRICT_ACCESS") restrictAccess = (integer)val;
			...
			if (llGetSubString(line, 0, 0) != "#")
			{
				...
				if (llList2String(tok,1) != "")
				{
					...
					// Now check for language translations
						 if (cmd == "TXT_ACCESS")          TXT_ACCESS = val;
					else if (cmd == "TXT_ADULTS_GIVE")     TXT_ADULTS_GIVE = val;
					...
					else if (cmd == "TXT_CHANGE_NAME")     TXT_CHANGE_NAME = val;
					...
					else if (cmd == "TXT_FEEDER")          TXT_FEEDER = val;
					...
					else if (cmd == "TXT_RESTRICTED")      TXT_RESTRICTED = val;
...
saveState()
{
	...
	if (status != "waitStatusNC")
	{
		...
		codedDesc1 += (string)(llRound(VERSION*100))+";" +(string)RADIUS+";" +SURFACE+";" +(string)CHATTY+";" +languageCode+";" +(string)givenBirth+";" +(string)epoch;
		...
		string codedDesc2 = "DP;" +(string)labelType+";" +neatVector(initpos)+";" +qsFloat2String(scaleFactor, 1, FALSE) +";" +(string)AN_AUTO_POO +";" +neatRotation(savedRot) +";" +neatVector(savedPos) +";" +(string)SLEEP_MODE +";" +(string)feedPet +";" +(string)IMMOBILE +";" +(string)restrictAccess +";" +AN_FEEDER;
		...
		if (isHuman == FALSE)
		{
			...
			debug("codedDesc2 (prim:" +(string)dataPrimNumber + "\n"+codedDesc2);
		}
	}
}
...
loadState(integer forceNC)
{
	...
	if (status != "waitStatusNC")
	{
		...
		else if ((llGetSubString(llGetObjectDesc(), 0, 0)  == "A") || (llGetSubString(llGetObjectDesc(), 0, 0)  == "X"))
		{
			...
			debug("DataPrim:"+(string)dataPrimNumber +"\n"+ llDumpList2String(desc, "|"));
		}

		if ((llList2String(desc, 0) == "A") || (llList2String(desc, 0) == "X"))
		{
			if (llList2String(desc, 0) == "X")
			{
				isPet = TRUE;
			}
			else
			...
			else
			{
				...
				epoch = 1; // Fall back to child in case not stored later
				createdTs = llList2Integer(desc, 4);

				// 5 is the chan which is generated automatically
				...
				if (llList2Integer(desc, 11) != 0)
				{
					...
					// Even newer versions of scripts save more data on a data prim
					if (llList2String(desc, 18) == "DP")
					{
						givenBirth = llList2Integer(desc, 16);
						epoch = llList2Integer(desc, 17);
						// 18 is 'DP' if we are here
						...
						restrictAccess = llList2Integer(desc, 28);
						AN_FEEDER = llList2String(desc, 29);
					}
				}
			}
		}
...
death(integer keepBody)
{
	...
	if (SKIN_OBJECT != "")
	{
		llMessageLinked(LINK_SET, 0, "REZ_PRODUCT|" +PASSWORD +"|" +(string)NULL_KEY +"|" +SKIN_OBJECT, NULL_KEY);
	}
}
...
showAlphaSet(integer newEpoch)
{
	...
	else if (newEpoch == 1)
	{
		...
		setAlphaByName("egg_prim", 0.0);
		setAlphaByName("adult_prim", 0.0);
		setAlphaByName("adult_male_prim", 0.0);
		setAlphaByName("adult_female_prim", 0.0);
		setAlphaByName("adult_random_prim", 0.0);
		setAlpha(ADULT_FEMALE_PRIMS+ADULT_MALE_PRIMS+ADULT_RANDOM_PRIMS, 0.0);// Legacy
	...
	{
		llSetLinkPrimitiveParamsFast(LINK_ALL_CHILDREN, [PRIM_COLOR, ALL_SIDES, WHITE, 1.]);
		setAlpha(CHILD_PRIMS+ADULT_RANDOM_PRIMS+ADULT_FEMALE_PRIMS+ADULT_MALE_PRIMS, 0.0);
		setAlphaByName("child_prim", 0.0);
		setAlphaByName("egg_prim", 0.0);

		if (llFrand(1.0) <0.5)
		{
			setAlpha(ADULT_RANDOM_PRIMS, 1.0);
			setAlphaByName("adult_random_prim", 1.0);
		}
		...
		if (sex == "Female")
		{
			setAlpha(ADULT_FEMALE_PRIMS, 1.0);
			setAlphaByName("adult_female_prim", 1.0);
			setAlphaByName("adult_male_prim", 0.0);
		}
		else
		{
			setAlpha(ADULT_MALE_PRIMS, 1.0);
			setAlphaByName("adult_male_prim", 1.0);
			setAlphaByName("adult_female_prim", 0.0);
		}
...
refresh()
{
	// Checks if day or night and sets 'sleeping' accordingly
	qlDayCheck();

	debug("REFRESH: Status=" +status + "  Sleeping="+(string)sleeping);
	...
	// Pets stop aging once they become adults
	if ((isPet == FALSE) || (epoch != 2)) age = (ts-createdTs);
	...
	else
	{
		...
		if (epoch == 1 && days  > (lifeTime*CHILDHOOD_RATIO/86400.0))
		{
			epoch = 2; // Child --> adult
			showAlphaSet(epoch);
			FEEDAMOUNT  = 2.0 * FEEDAMOUNT;
			WATERAMOUNT = 2.0 * WATERAMOUNT;
		}

		if (food < 0 && water <0)
		{
			say(0, AN_BAAH+", " +TXT_HUNGRY_THIRSTY, NULL_KEY);
		}
		else if (food < 0)
		{
			say( 0,AN_BAAH+", " +TXT_HUNGER, NULL_KEY);
		}
		else if (water < 0)
		{
			say( 0, AN_BAAH+", " +TXT_THIRST, NULL_KEY);
		}
		...
		else
		{
			...
			if (epoch >1)
			{
				float p = 100.0 * (ts - milkTs)/MILKTIME;

				if (p > 100) p = 100;

				// MILK OBJECT STATUS
				if (AN_HASMILK && sex == "Female" && sleeping == FALSE && giveProducts == TRUE)
				{
					if (LAYS_EGG==1)
					{
				...
				// WOOL OBJECT STATUS
				p = 100.0 * (ts - woolTs)/WOOLTIME;

				if (p > 100) p = 100;

				if ((AN_HASWOOL == TRUE) && (sleeping == FALSE) && (giveProducts == TRUE))
				{
				...
				p = 100.0 * (ts - manureTs)/MANURETIME;

				if (p > 100) p = 100;

				if ((AN_HASMANURE == TRUE) && (sleeping == FALSE) && (giveProducts == TRUE))
				{
		...
		else
		{
			...
			if (sleeping == FALSE)
			{
				...
				else
				{
					...
					if (labelType == 2)
					{
						llSetText(tmpStr, BLACK, 0.5);
					 }
					else if (labelType == 1)
					{
						llSetText(humanStr, PURPLE, 1.0);
					}
					else
					{
						llSetText(str, PURPLE, 1.0);
					}
				}
			...
			// if animal is sleeping then let them sleep!
			else
			{
				setPose(down);
				llSetText(TXT_SLEEPING, PURPLE, 1.0);
			}
		}
...
// Returns TRUE if it is daytime, FALSE otherwise. Also sets integer 'sleeping' to true or false if SLEEP_MODE is true (i.e. animal does sleep at night)
integer qlDayCheck()
{
	integer result = 1;
...
rezManure(key id)
{
	if ((giveProducts == TRUE) && (initialised == TRUE))
	{
		string rezObj = MANURE_OBJECT;

		if ((AN_AUTO_POO == TRUE) && (llGetInventoryType(rezObj+"-auto") == INVENTORY_OBJECT))
		{
			rezObj += "-auto";
		}
...
showOptionsMenu(key id)
{
	...
	if ((epoch == 2) || (isPet == TRUE) || (isHuman == TRUE))
	{
		if ((restrictAccess == FALSE) || (id == llGetOwner())) opts += TXT_SCALE;
	}

	if (id == llGetOwner())
	{
		if (restrictAccess == FALSE) opts+= "+"+TXT_ACCESS; else opts+= "-"+TXT_ACCESS;
	}

	if (epoch ==2)
	{
	   if ((restrictAccess == FALSE) || (id == llGetOwner())) opts += TXT_BUTCHER;
	}

	if (llGetInventoryType(feedersNC) == INVENTORY_NOTECARD) opts += TXT_FEEDER;
	opts += TXT_FLOAT_TEXT;
	...
	if (labelType == 0)
	{
		prompt += TXT_FLOAT_FULL;
	}
	else if (labelType == 1)
	{
		prompt += TXT_FLOAT_SHORT;
	}
	else
	{
		prompt +=TXT_FLOAT_NONE;
	}
	...
	if (AN_HASMANURE && giveProducts == TRUE)
	{
	...
	if ((isPet == FALSE) || (feedPet == TRUE)) 
	{
		prompt += "\n"+TXT_FEEDER+": " +AN_FEEDER;
	}
...
doTouch(key toucher)
{
	...
	else if ((toucher == llGetOwner()) || (llSameGroup(toucher) == TRUE) || (osIsNpc(toucher) == TRUE))
	{
		...
		opts += [TXT_STROKE, TXT_INFO,  TXT_LANGUAGE, TXT_OPTIONS];
		...
		if (IMMOBILE <= 0)
		{
			if (isAttached == FALSE)
			{
				...
				{
					if ((restrictAccess == FALSE) || (toucher ==llGetOwner())) opts += TXT_FOLLOW_ME;
				}
	   ...
	   if ((sex == "Female" && epoch == 2) && (isHuman == FALSE)  && (sleeping == FALSE))
	   {
		   ...
		   {
				if ((restrictAccess == FALSE) || (toucher ==llGetOwner())) opts += TXT_MATE;
		   }
	   }
	   ...
	   if (epoch == 2)
	   {
			if (giveProducts == TRUE)
			{
				...
				if ((ts - woolTs > WOOLTIME) && (AN_HASWOOL >0) && (sleeping == FALSE))
				{
					opts += TXT_WOOL;
				}

				if ((ts - manureTs > MANURETIME) && (AN_HASMANURE >0) && (sleeping == FALSE))
				{
					opts += TXT_GET_MANURE;
				}
			}
...
default
{
	state_entry()
	{
		...
		lifeTime = (integer)((float)LIFETIME*( 1.0 + llFrand(0.1)));
	...
	object_rez(key id)
	{
		...
		if (llKey2Name(id) == llGetObjectName()) //Child
		{
			...
			if (AN_HASGENES == TRUE)
			{
				if (llFrand(2.0)<1)
				{
					newName = name+" "+fatherName+"son";
				}
				else
				{
					newName = fatherName +" Fitz" +llToLower(name);
				}
			...
			llSleep(1);
			messageObj(id, "INIT|"+PASSWORD+"|"+babyParams+"|"+languageCode+"|"+SURFACE+"|"+(string)CHATTY+"|"+(string)labelType+"|"+(string)RADIUS+"|"+(string)AN_AUTO_POO+"|"+AN_FEEDER+"|"+(string)SLEEP_MODE +"|" +(string)saveStatus +"|" +(string)restrictAccess +"|" +(string)isPet +"|" +(string)giveProducts);
		}
		...
		else
		{
			...
			if ((MEAT_OBJECT=="" || (deathFlags&2)) && (SKIN_OBJECT=="" || (deathFlags&4)))
			{
				...
				if (deathFlags&1)
				{
					...
					{
						cause = TXT_STARVATION;
					}
					else
					{
						cause = TXT_DEHYDRATION;
					}
			...
			if (isAttached == FALSE)
			{
				...
				else
				{
					...
					else if (followUser == NULL_KEY)
					{
						...
						{
							llSetTimerEvent(lifeFactor+ (integer)llFrand(20));
						}
	...
	listen(integer c, string n ,key id , string m)
	{
		...
		else if (m == TXT_INFO)
		{
			...
			if (epoch < 2)
			{
				str += ", " +TXT_BECOME_ADULT +" " +(string)llRound(lifeTime*CHILDHOOD_RATIO/86400.0) +" " +TXT_DAYS;
			}
			...
			if (isHuman == FALSE)
			{
				...
				if (AN_HASMILK && giveProducts)
				{
					str += TXT_FEMALES_GIVE +" " +MILK_OBJECT+ " " +TXT_EVERY +" " +(string)llRound(MILKTIME/3600)+" " +TXT_HOURS + ". ";
				}

				if (AN_HASMANURE && giveProducts)
				{
					str += TXT_ADULTS_GIVE +" " +MANURE_OBJECT +" " +TXT_EVERY + " "+(string)llRound(MANURETIME/3600)+" " +TXT_HOURS +". ";
				}

				if ((LAYS_EGG == 0) && (giveProducts == TRUE))
				{
					if (sex == "Female")
					{
						str += TXT_PREGNANCY +" " +(string)(PREGNANT_TIME/86400) +" " +TXT_DAYS +". ";
					}
				}
			...
			if (AN_HASWOOL && giveProducts)
			{
				str += TXT_ADULTS_GIVE  +" " +WOOL_OBJECT +" " +TXT_EVERY +" " +(string)llRound(WOOLTIME/3600)+" " +TXT_HOURS + ". ";
			}
			...
			if ((isPet == FALSE) || (isPet == TRUE && feedPet == TRUE))
			{
				...
				{
					str += TXT_HUNGRY +": " +(string)llRound(100-tmp) +"% \t";
				}
				...
				{
					str +=TXT_THIRSTY +": " +(string)llRound(100-tmp) +"%";
				}
			}

			str += "\n[" +TXT_ANIMAL_VERSION +" " +qsFloat2String(VERSION, 2, FALSE) +" ";
		...
		else if ((m == "-"+TXT_AUTO_MANURE) || (m == "+"+TXT_AUTO_MANURE))
		{
			...
			if (AN_AUTO_POO == TRUE)
			{
				llRegionSayTo(id, 0, TXT_AUTO_MANURE +": " +TXT_ON);
			}
			else
			{
				llRegionSayTo(id, 0, TXT_AUTO_MANURE +": " +TXT_OFF);
			}
		...
		else if ((m == "-"+TXT_SLEEP_MODE) || (m == "+"+TXT_SLEEP_MODE))
		{
			...
			if (SLEEP_MODE == TRUE)
			{
				...
				if (qlDayCheck() == FALSE)
				{
					...
					{
						llMessageLinked(LINK_SET, 1, "EXPRESSION|SLEEP", "");
					}
				}
				else
				{
		...
		else if ((m == "-"+TXT_ACCESS) || (m == "+"+TXT_ACCESS))
		{
		...
		else if ((m == TXT_SET_NAME) ||(m == TXT_CHANGE_NAME))
		{
		...
		else if (m == TXT_STROKE)
		{
			if (sleeping == FALSE)
			{
				...
				if (food < 0 && water <0)
				{
					say(0, AN_BAAH+", " +TXT_HUNGRY_THIRSTY, id);
				}
				else if (food < 0)
				{
					say( 0,AN_BAAH+", " +TXT_HUNGRY, id);
				}
				else if (water < 0)
				{
					say( 0, AN_BAAH+", " +TXT_THIRSTY, id);
				}
		...
		else if (m == TXT_FEEDER)
		{
			status = "waitFeeder";

			if (llGetInventoryType(feedersNC) == INVENTORY_NOTECARD)
			{
				list lines = llParseString2List(osGetNotecard(feedersNC), ["\n"], []);
				integer i;
				...
				for (i=0; i < count; i++)
				{
					if (llGetSubString(llList2String(lines,i), 0, 0) !="#")
					{
						feeders += llList2String(lines,i);
					}
				}

				debug("FEEDERS\n"+llDumpList2String(feeders, "\n"));
				startOffset = 0;
				multiPageMenu(id, TXT_FEEDER, feeders);
			}
		}
		...
		else if (status == "waitFeeder")
		{
			AN_FEEDER = m;
			llRegionSayTo(id, 0, TXT_FEEDER+" = " +m);
			status = "";
			saveState();
			refresh();
		}
	...
	dataserver(key kk, string m)
	{
		...
		if (llList2String(tk,1) == PASSWORD)
		{
			...
			else if (cmd == "VERSION-CHECK")
			{
				...
				integer ver = (integer)(VERSION*100);
				...
				if (llGetInventoryType("angel") == INVENTORY_TEXTURE)
				{
					answer+= "angel,";
				}
			...
			else if (cmd == "MATEME" ) //Male part
			{
					...
					list kf;
					vector mypos = llGetPos();
					vector v = llList2Vector(ud, 1) + <-0.3,  0.0, 0.3> * llList2Rot(ud,2);
			...
			else if (cmd == "INIT")
			{
				...
				// INIT|PASSWORD|geneA|fatherGene|newName|languageCode|SURFACE|CHATTY|labelType|RADIUS|AN_AUTO_POO|AN_FEEDER|SLEEP_MODE|saveStatus|restrictAccess|isPet|giveProducts
				// If command sent from rezzer geneA and fatherGene are 0 and also non-override values are ""
				
				// We start with giveProducts = -1 so as not to produce manure on Rez if giveProducts = 0
				if (llList2String(tk, 16) != "") giveProducts = llList2Integer(tk, 16);
				if (llList2String(tk, 15) != "") isPet = llList2Integer(tk, 15);
				if (llList2String(tk, 14) != "") restrictAccess = llList2Integer(tk, 14);
				...
				if (llList2String(tk, 10) != "") AN_AUTO_POO = llList2Integer(tk, 10);
				if (llList2String(tk,  9) != "") RADIUS = llList2Integer(tk, 9);
				if (llList2String(tk,  8) != "") labelType = llList2Integer(tk, 8);
				if (llList2String(tk,  7) != "") CHATTY = llList2Integer(tk, 7);
			...
			else if (cmd == "PETIFY")
			{
				...
				if (llList2Integer(tk, 2) == 1)
				{
					...
					if (age > lifeTime)
					{
						age = lifeTime - 172800; // take off 2 days
					}
