/*
 * CHANGE LOG
    * Issue with medicine under provisions going negative see  https://github.com/punyauk/VivoSim/issues/8
	* Allow Indicator to still work (showing just hunger & thirst plus OCC/AFK options) if Health mode is off
	* Debug option to choose between output via llOwnerSay or use debug chan.
	* Fix to nutrion values for SF Beef Cherry Sauce
*/

float   VERSION = 6.08;			// 7 March 2025  6.08
...
integer ownerSay = TRUE;					//  Use 1 to output debug via llOwnerSay, or 0 to output it to the DEBUG_CHANNEL
//
...
integer checkIndicator(string indName)
{
    ...
    if (indName == statusIndicator)
    {
        if (result == FALSE)
		{
			llMessageLinked(LINK_ALL_CHILDREN, 0, "COMMS|INDICATOR|0", "");
		}
		else
		{
...
setConfig(string line)
{
    ...
    if (llList2String(tok,1) != "")
    {
		...
		else if (cmd == "DEBUG_SAY")
		{
			ownerSay = (integer)val;
		}
...
floatText(string msg, vector colour, integer raw)
{
    ...
    if (msg != lastText)
    {
        ...
        if (MY_HUD == TRUE)
        {
            if (raw == TRUE)
			{
				sendMsg = wasSpaceWrap(msg, "\n", 64) +"|" +(string)colour;
			}
			else
			{
				sendMsg = msg +"|" +(string)colour;
			}
...
setHealthMode(integer level)
{
    ...
    if (healthMode == 0)
    {
        ...
        if (checkIndicator(statusIndicator) == TRUE)
		{
			...
			messageObj(indicatorKey, "HEALTHOFF|" +PASSWORD);
		}
    }
    else
    {
		if (checkIndicator(statusIndicator) == TRUE)
		{
			messageObj(indicatorKey, "HEALTHON|" +PASSWORD);
		}
