...
// ---------------------------------------
float version = 5.2;   //  31 January 2022
//
...
string PASSWORD="*";
integer quinActive;                     // TRUE when communication with Quintonia server established
integer validated;                      // TRUE when active user is known to be linked at a Quintonia account
string oswToken;
key dlgUser;
key userToPay;
key httpID = NULL_KEY;
...
activate()
{
    ...
    // Talk to PHP script and check comms is okay with Quintonia server then activate
    status = "activation";
...
{
    httpID = llGetKey();
    llMessageLinked(LINK_SET, 1, "CMD_POST|"+msg, httpID);
}
...
default
{
    ...
    {
        userToPay= id;
        debug("link_message:" + msg +"  Num:" +(string)num +"  User: "+(string)userToPay);
        ...
        else if (cmd == "HTTP_RESPONSE")
        {
            if (id == httpID)
            {
                ...
                {
                    //  Communication established okay with Quintonia server
                    quinActive = TRUE;
                    llMessageLinked(LINK_SET, 0, "COMMS|WEB|1", "");
                    // Now check if the user has linked this avatar to a Quintonia account
                    validated = FALSE;
                ...
                else if (cmd == "POINTTALLY")
                {
                    ...
                    {
                        validated = TRUE;
                        llMessageLinked(LINK_SET, 1, "VALIDATED", userToPay);
                        status = "initCoin";
            ...
            {
                //debug("Points script:HTTP_RESPONSE LINK not matching send key");
            }
        }
    }
